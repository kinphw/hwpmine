...
PyInstaller spec 에 name='python' 으로 지정돼 있어 dist/python.exe 가
생성된다. 운영환경 DRM 솔루션이 프로세스 basename 만으로 화이트리스트
판정하기 때문에 'python.exe' 라는 이름이 필수.

배포물 사용 방법은 단순: 사용자가 zip 풀고 python.exe 더블클릭 = 끝.
별도 install 스크립트 없음.

수행 작업:
    1) src/docmine/__init__.py 의 버전 읽기
    2) dist/python.exe 가 있는지 확인
    3) release/docmine_v<버전>/ 에 exe, .env.example, README.md 모음
...
def find_exe() -> Path:
    ...
    if not exe.exists():
        sys.exit(
            f"[오류] 실행파일이 없습니다: {exe}\n"
            f"       먼저 다음 명령으로 빌드하세요:\n"
            f"         pyinstaller docmine.spec --clean --noconfirm"
        )
    return exe
